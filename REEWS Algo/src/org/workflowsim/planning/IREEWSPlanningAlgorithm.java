...
public class IREEWSPlanningAlgorithm extends BasePlanningAlgorithm
{
    ...
    public IREEWSPlanningAlgorithm() throws NullPointerException{
   ...
   /* public void estCal()
    ...
    public void clusterUtil(Task t,Map<Task,Integer> map,Map<Task,Boolean> assign)
    {
        //clust.add(t);
        assign.put(t,true);
        ...
        for(Task task : l1)
        {
            ...
            {
                Task parent=null;
                double maxi_tc=0.0;//max transfer cost
                for(Task t2 : task.getParentList())
                {
                    if(transferCosts.get(t2).get(task)>=maxi_tc)
                    {
                        parent=t2;
                        maxi_tc=transferCosts.get(t2).get(task);
                    }
                }
                if(parent!=null)
                {
                    clusters.get(map.get(parent)).add(task);
                  //  assign.put(task, true);
                    map.put(task, map.get(parent));
                    //clusterUtil(task,map,assign);
                }
            }
        }
    }
...
//                t.setVmFreq(freq.get(vm));
        ...
        tot_relia/=k;
        Output.reliability=tot_relia;
        System.out.println("Overall Reliability="+tot_relia);
